---
title: "Jeff Cup Auction History"
format: html
editor: visual
---
...
```{r}
model_qb <- draft_values %>% 
  filter(position == "QB") %>% 
  loess(bidAmount ~ positionRank, data = .)

pred_qb <- tibble(
  position = "QB",
  rank = 1:16,
  amount = predict(model_qb, newdata = 1:16, SE = TRUE)
)

model_rb <- draft_values %>% 
  filter(position == "RB") %>% 
  loess(bidAmount ~ positionRank, data = .)
...
pred_rb <- tibble(
  position = "RB",
  rank = 1:49,
  amount = predict(model_rb, newdata = 1:49, SE = TRUE)
)

model_wr <- draft_values %>% 
  filter(position == "WR") %>% 
  loess(bidAmount ~ positionRank, data = .)
...
pred_wr <- tibble(
  position = "WR",
  rank = 1:53,
  amount = predict(model_wr, newdata = 1:53, SE = TRUE)
)

model_te <- draft_values %>% 
  filter(position == "TE") %>% 
  loess(bidAmount ~ positionRank, data = .)
...
pred_te <- tibble(
  position = "TE",
  rank = 1:14,
  amount = predict(model_qb, newdata = 1:14, SE = TRUE)
)

pred_values <- bind_rows(
  pred_qb,
  pred_rb,
  pred_wr,
  pred_te
...
```{r}
ss <- "https://docs.google.com/spreadsheets/d/1Pt50fh9AM-a9giQCuQUrkI2NbROHlBGsb4PPHk3utFI"
pred_values %>% 
  googlesheets4::sheet_write(ss, "Predicted Costs")
```
