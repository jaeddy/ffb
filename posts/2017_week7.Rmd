---
title: "Jefferson Cup 2017 — Week 7 Dashboard"
output: 
...
```{r}
pr_week7 <- get_powerrankings(2017, 7, data_dir = "../data")
```
...
current_order <- trends_df %>% 
  filter(week == "WEEK 7", trend_type == "power_rank") %>% 
  arrange(rank) %>% 
  .[["Team"]]
```
...
p <- trends_df %>% 
  ...
  scale_x_continuous(breaks = 1:8,
                     labels = levels(trends_df$week)[1:8],
                     limits = c(1, 8)) +
