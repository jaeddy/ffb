---
title: "Jefferson Cup 2017 — Week 13 Dashboard"
output: 
...
```{r}
pr_week13 <- get_powerrankings(2017, 13, data_dir = "../data")
```
...
current_order <- trends_df %>% 
  filter(week == "WEEK 13", trend_type == "power_rank") %>% 
  arrange(rank) %>% 
  .[["Team"]]
```
...
p <- trends_df %>% 
  ...
  scale_x_continuous(breaks = 1:14,
                     labels = levels(trends_df$week)[1:14],
                     limits = c(1, 14)) +
