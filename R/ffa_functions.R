...
optimizeTeam <- function(points=optimizeData$points, playerCost=optimizeData$inflatedCost, maxRisk=maxRisk){ #can change points, cost, or risk
  ...
  A <- rbind(as.numeric(optimizeData$pos == "QB"), # num QB
             ...
             as.numeric(optimizeData$pos == "DST"),# num DST
  ...
  sol$playerInfo <- as.data.frame(cbind(optimizeData[sol$solution == 1, "player"],
                                        optimizeData[sol$solution == 1, "pos"],
                                        round(points[sol$solution == 1], 2), 
                                        round(optimizeData[sol$solution == 1, "risk"], 2), 
                                        playerCost[sol$solution == 1]))
  names(sol$playerInfo) <- c("player","pos","points","risk","cost")
